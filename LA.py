...
def add_vectors(vector_a: list[float], vector_b: list[float]) -> list[float]:
    ...
    Returns:
        The sum of the inputs vectors stored as a list.
...
def scalar_vector_Multi(vector_A: list[float], scalar_A) -> list[float]:
    ...
    Returns:
        The multiplication of the input vector and scalar.
...
def matrix_vector_Multi(matrix_A, vector_A):
    ...
    if (len(matrix_A) != len(
            vector_A)):  # Added a if statement to check if the columns from matrix_A are the same as the rows in vector_A.
        result_Vector = "Error: The dimensions are not compatible"
